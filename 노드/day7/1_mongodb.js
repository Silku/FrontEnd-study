...
//mongdb 연결함수
function connectDB(){
const datbaseURL="mongodb://localhost:27017";
MongoClient.connect(datbaseURL,(err,db)=>{
    if(!err){
...
router.route('/member/edit').post((req, res) => {
    ...
    if(database){
        ...
        res.write('<p>데이터베이스 연결에 실패했습니다.</p>');
        res.end(); 
    }
...
const joinMember = function(database, userid, userpw, name, age, callback){
    ...
    members.insertMany([{userid:userid, userpw:userpw, name:name, age:age}], (err, result) => {
        if(!err){
            if(result.insertedCount > 0){
...
const loginMember = function(database, userid, userpw, callback){
    ...
    members.find({userid:userid, userpw:userpw}).toArray((err, result) => {
          //find는 여러개의 객체를 찾을 수 있다, 그래서 toArray()사용
        if(!err){
            if(result.length > 0){
...
const editMember = function(database, userid, userpw, name, age, callback){
    ...
    const members = database.collection('member');
    members.updateOne({userid:userid},{$set:{userid:userid, userpw:userpw, name:name, age:age}},(err,result)=>{
        if(!err){
            if(result.modifiedCount > 0){
